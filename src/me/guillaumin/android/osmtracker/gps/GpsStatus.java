...
public final class GpsStatus {
	...
	/**
	 * Used for receiving RAW data or NMEA sentences from the GPS.
	 */
    ...
    void setStatus(android.location.GpsStatus status) {
    	mTimeToFirstFix = status.getTimeToFirstFix();

    	for (GpsSatellite s0: mSatellites) {
    		s0.mValid = false;
    	}

    	for (android.location.GpsSatellite satellite: status.getSatellites()) {
    		int prn = satellite.getPrn(); 
    		if (prn > 0 && prn <= mSatellites.length) {
    			mSatellites[prn-1].setStatus(satellite);
    		}
    	}
    }
